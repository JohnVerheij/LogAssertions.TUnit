...
**Requirements:** TUnit 1.65.51 or later, .NET 10. The package is AOT-compatible, trimmable, and uses no reflection in the assertion path.
...
- Wording edits to a template stop breaking tests: the test asserts intent (this event), not
  prose. Renaming the definition or changing its signature breaks the capture lambda at
  compile time (a positional capture call does not bind to a renamed or restructured method).
