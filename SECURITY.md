...
LogAssertions / LogAssertions.TUnit is a personal-name open-source project distributed under the [MIT license](LICENSE). Although the package is intended for use **only in test projects** (see the scope blockquote on every README), security reports are still welcome and treated as a priority.
...
1. Navigate to the repository's [Security tab](https://github.com/JohnVerheij/LogAssertions.TUnit/security)
...
|---|---|---|
| `0.4.x` (LogAssertions.TUnit) | **Current** | ✅ Yes |
| `0.3.x` | Previous stable | ✅ Yes (security fixes only; no new features) |
| `0.2.x` | Older | ❌ No — please upgrade |
| `< 0.2.0` | Pre-stable | ❌ No |
...
- The shipped `LogAssertions` and `LogAssertions.TUnit` NuGet packages
...
- Vulnerabilities in **direct dependencies** (`Microsoft.Extensions.Diagnostics.Testing`, `TUnit.*`, etc.) — report to the relevant upstream maintainer; we will pull in their fix once released
...
Replace `<package>` (one of `LogAssertions` or `LogAssertions.TUnit`) and `<version>` (e.g. `0.4.0`) with the artifact you downloaded:

```bash
# Verify SLSA v1.0 build provenance: was this nupkg built from this audited source tree?
gh attestation verify ./<package>.<version>.nupkg --repo JohnVerheij/LogAssertions.TUnit

# Verify the CycloneDX SBOM: is this BOM the authentic SBOM for that nupkg?
gh attestation verify ./<package>.<version>.nupkg \
    --repo JohnVerheij/LogAssertions.TUnit --predicate-type https://cyclonedx.org/bom
```
...
| Build → SBOM (sibling) | CycloneDX dotnet tool + `actions/attest-sbom@v2` | CycloneDX 1.6 + Sigstore signature |
