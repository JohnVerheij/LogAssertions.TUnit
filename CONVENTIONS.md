...
Every NEW public string-matching API requires the caller to pass `StringComparison` explicitly.
...
enforces this on internal code via MA0006 / MA0001.

A small number of legacy single-arg `(string)` overloads from earlier 0.x releases (e.g.
`LogFilter.WithExceptionMessage(string)`) remain available for binary-compat with the previous
shipped baseline. They are marked `[Obsolete(error: false)]`, default to
`StringComparison.Ordinal`, and are removed in their two-minor-cycle window. Each `[Obsolete]`
entry points at the explicit-comparison replacement; new code should never call the legacy
overload.
